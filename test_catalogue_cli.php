<?php
/**
 * test_catalogue_cli.php
 * Tests complets CLI (sans authentification web)
 */
...
$files = [
    ['admin/catalogue/produits.php', 'file'],
    ['admin/catalogue/produit_edit.php', 'file'],
    ['admin/catalogue/produit_delete.php', 'file'],
    ['admin/catalogue/categories.php', 'file'],
    ['admin/catalogue/README.md', 'file'],
    ['uploads/catalogue/', 'dir']
];
...
foreach ($files as $item) {
    ...
    $path = __DIR__ . '/' . $file;
    if ($type === 'dir') {
        test("Dossier $file existe", is_dir($path), "Dossier non trouvé: $path");
        test("Dossier $file accessible en écriture", is_writable($path), "Pas d'accès écriture");
    } else {
        test("Fichier $file existe", file_exists($path), "Fichier non trouvé: $path");
        if (preg_match('/\.php$/', $file)) {
            $output = shell_exec("php -l \"$path\" 2>&1");
            test("Fichier $file syntaxe PHP valide", strpos($output, 'No syntax errors') !== false, "Erreur syntaxe");
        }
    }
}
...
if ($test_category_id) {
    try {
        ...
        // Vérifier que le slug existe (elle doit contenir au moins partie du nom ou uniqid)
        test("Slug généré et unique", $cat && !empty($cat['slug']), "Slug vide: " . ($cat['slug'] ?? 'null'));
    } catch (Exception $e) {
...
if ($test_product_id) {
    try {
        ...
        $stmt->execute([$test_product_id]);
        $prod = $stmt->fetch();
        test("Modifier prix produit", $prod && abs(floatval($prod['prix_unite']) - $new_prix) < 0.01, "Modification prix échouée");
    } catch (Exception $e) {
...
if ($test_product_id) {
    try {
        ...
        test("Code unique (doit échouer)", true, "OK: Contrainte respectée");
    }
}
...
try {
    ...
    test("Catégorie requise (doit échouer)", true, "OK: Contrainte respectée");
}
...
if ($test_category_id && $test_product_id) {
    try {
        ...
        test("Empêcher suppression catégorie avec produits", true, "OK: Contrainte FK respectée");
    }
}
...
try {
    ...
    test("Requête liste produits avec JOIN", is_array($produits), "Erreur requête");
} catch (Exception $e) {
...
try {
    ...
    $result = $stmt->fetch();
    test("Filtrage produits actifs", isset($result['cnt']), "Erreur filtrage");
} catch (Exception $e) {
