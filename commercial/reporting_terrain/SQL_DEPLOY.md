# 🗄️ Script SQL à exécuter dans phpMyAdmin Bluehost

## Instructions d'exécution

1. Se connecter à **cPanel Bluehost**
2. Ouvrir **phpMyAdmin** (section Databases)
3. Sélectionner la base de données **`kms_gestion`** dans la colonne de gauche
4. Cliquer sur l'onglet **SQL** en haut
5. **Copier-coller TOUT le contenu ci-dessous** dans la zone de texte
6. Cliquer sur **Go** (Exécuter)
7. Vérifier le message de succès : "7 requêtes ont été exécutées avec succès"

---

## ⚠️ ATTENTION

- Ce script crée 7 nouvelles tables
- Aucune donnée existante ne sera modifiée
- Les clauses `IF NOT EXISTS` évitent les erreurs si vous exécutez le script plusieurs fois
...
Exécutez cette requête pour vérifier que les 7 tables ont été créées :

```sql
SHOW TABLES LIKE 'terrain_reporting%';
```

**Résultat attendu :** 7 lignes affichées
...
**Solution :** Normal si vous exécutez le script plusieurs fois. Les tables existent déjà, aucun problème.

### Erreur : "Cannot add foreign key constraint"
**Solution :** Vérifier que la table `utilisateurs` existe dans votre base de données.

### Erreur : "Access denied"
**Solution :** Vérifier que votre utilisateur MySQL a les droits CREATE TABLE sur la base.

---

**Prêt à copier-coller dans phpMyAdmin**
